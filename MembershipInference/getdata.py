...
def initializeData(args,root,num_shadow,dataFolderPath = './data/'):
    ...
    try:
        os.makedirs(dataPath+'/'+ str(args.classes)+ '/'+str(args.cluster))

    except OSError:
        # assert(0)
        pass

    np.savez(dataPath +'/'+ str(args.classes)+ '/'+str(args.cluster)+'/targetTrain.npz', toTrainData, toTrainLabel)
    np.savez(dataPath + '/'+ str(args.classes)+'/'+str(args.cluster)+'/targetTest.npz',  toTestData, toTestLabel)
    np.savez(dataPath + '/'+ str(args.classes)+'/'+str(args.cluster)+'/shadowTrain.npz', totalshadowtrain, totalshadowtrainlabel)
    np.savez(dataPath + '/'+ str(args.classes)+'/'+str(args.cluster)+'/shadowTest.npz',  totalshadowtest,totalshadowtestlabel)
